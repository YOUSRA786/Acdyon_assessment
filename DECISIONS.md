...
I've adopted an event-driven paradigm using WebSockets/Socket.IO for real-time elements of the Code Brawl platform.

My primary motivation was that the core of the platform centers around live coding matches. Elements like the match status, the opponent's actions, code changes, and the game timer need to be updated instantly without requiring constant page refreshes from the user.

My considered alternative was standard HTTP polling, where the frontend constantly inquires the backend about changes. This approach felt undesirable due to the sheer volume of requests and the lack of a responsive, multiplayer feel.

Additionally, heavier computations were kept separate from the real-time communication. Aspects like code submissions and other asynchronous processes can be managed without impacting the live match.

---

### 2. One trade-off made under the time limit

Due to time constraints, I prioritized getting the core Code Brawl experience functional over developing every feature to production quality.

The main focus was on the platform's core functionality: landing page, challenges, matchmaking, live coding, match outcomes, rankings, and general UI/UX.

Consequently, sections such as advanced error handling, thorough testing, monitoring, and large-scale optimization are less mature than in a production application.

Given more time, I would have emphasized test coverage, edge case management, backend hardening, code execution security, and concurrent user performance testing.

---

### 3. Where did you use AI tools, and what did you personally verify or change afterward?

I primarily utilized AI tools to brainstorm UI concepts, design ideas, and implementation suggestions.

For visual research, I referred to platforms like Lapa and Dribbble to explore layouts for the landing page, color schemes, typography choices, animations, cards, and general visual aesthetics. I then employed AI tools to help translate some of these inspirations into the Code Brawl interface.

I always reviewed and modified the designs generated by AI rather than implementing them directly. I tweaked spacing, alignment, colors, animations, navigation actions, and the responsive structure to ensure the design aligned with the Code Brawl vision.

All main user flows and interactions were manually inspected to ensure the actual implementation matched the intended design.
